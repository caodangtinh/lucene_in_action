...
public class NearRealTimeTest {
	...
	public void testNearRealTime() throws CorruptIndexException, LockObtainFailedException, IOException, ParseException {
		...
		for (int i = 0; i < 10; i++) {
			...
			document.add(new Field("id", ""+i, Field.Store.NO, Field.Index.NOT_ANALYZED_NO_NORMS));
		...
		Document document =new Document();
		document.add(new Field("id", ""+11, Field.Store.NO, Field.Index.NOT_ANALYZED_NO_NORMS));
		...
		TopDocs topDocs2 = indexSearcher.search(query, 1);
